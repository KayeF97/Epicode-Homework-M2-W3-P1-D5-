...
//searching for TITLE and LOCATION  
// delete the "description", "requirements", "benefits" and "company_profile" fields from the results as to avoid visual clutter.
// results to be returned in this format : result: [], <<<< here will be the jobs
//count: 0 <<<< here will be the number or results




  //this loop returns all job titles w/locations 
//this loop now returns all job parameters  based on search for either title and location. returns answer in an array

  const query = "Craftsman"
  const result = [];

for (let i = 0; i < jobs.length; i++) {
  if(jobs[i].title.includes(query) || jobs[i].location.includes(query) ){

    result.push(jobs[i].job_id + " " + jobs[i].title + " " + jobs[i].location + " " + 
    jobs[i].department + " " + jobs[i].salary_range + " " + jobs[i].telecommuting + " " +
    jobs[i].has_company_logo + " " + jobs[i].has_questions + " " + jobs[i].employment_type + " "+
    jobs[i].required_experience + " " + jobs[i].required_education + " " + jobs[i].industry + " " + 
    jobs[i].function + " " + jobs[i].fraudulent);
